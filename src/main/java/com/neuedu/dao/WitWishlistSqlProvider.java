...
import com.neuedu.entity.WitWishlist;
import org.apache.ibatis.jdbc.SQL;

public class WitWishlistSqlProvider {
    public String insertSelective(WitWishlist record) {
        SQL sql = new SQL();
        sql.INSERT_INTO("wit_wishlist");
        
        if (record.getWitId() != null) {
            sql.VALUES("WIT_ID", "#{witId,jdbcType=INTEGER}");
        }
    ...
    public String updateByPrimaryKeySelective(WitWishlist record) {
        SQL sql = new SQL();
        sql.UPDATE("wit_wishlist");
        ...
        sql.WHERE("WIT_ID = #{witId,jdbcType=INTEGER}");
